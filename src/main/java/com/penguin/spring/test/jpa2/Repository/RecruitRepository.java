...
public interface RecruitRepository extends JpaRepository<Recruit, Integer> {
	...
	public List<Recruit> findByTypeOrSalaryBetween(String type, int start , int end);
	...
	@Query(value="SELECT * FROM `recruit` WHERE `salary` >= :salary AND `type` = :type AND `deadline` >= :deadline ORDER BY `salary` DESC" ,nativeQuery=true)
	public List<Recruit> findBySalaryAndTypeQuery(@Param("salary")int salary, @Param("type")String type,  @Param("deadline")String deadline);
